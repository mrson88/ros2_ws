...
parser.add_argument("--prototxt", default="MiAI_MobileNetSSD_Pi/MobileNetSSD_deploy.prototxt")
parser.add_argument("--weights", default="MiAI_MobileNetSSD_Pi/MobileNetSSD_deploy.caffemodel")
...
def do_detect(frame):
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence > 0.5:
            ...
            xcentre=(xLeftBottom+xRightTop)/2
            # Ve label cua doi tuong
            if class_id in classNames:
                label = classNames[class_id] + ": " + str(round(confidence*100,2))+' %'
                # hien thi len man hinh
                cv2.putText(frame, label, (xLeftBottom, yLeftBottom), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255),1)
        else:
            class_id=0
            confidence=0
            xcentre=100
        return  frame,class_id,confidence,xcentre
    #return  frame

# Mo video hoac webcam
if args.vid_file:
    cap = cv2.VideoCapture('ssd_mobilenet_model/videos/test3.mp4')
else:
    cap = cv2.VideoCapture('ssd_mobilenet_model/videos/test3.mp4')
