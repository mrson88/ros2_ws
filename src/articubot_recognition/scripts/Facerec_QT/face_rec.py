...
package_share_dir = get_package_share_directory("articubot_recognition")
IMG_SHAPE = [96, 112]
OBJ_THRES = 0.7
NMS_THRES = 0.4
VARIANCE = [0.1, 0.2]
...
db_dir = os.path.join(package_share_dir, "Facerec_QT/resources","database.db")
...
class FaceRecognition():
    ...
    def __init__(self, threshold, image_height, image_width):
        self.fd_model_dir=os.path.join(package_share_dir, "Facerec_QT/face_rec_models","ulffd_landmark.tflite")
        self.fe_model_dir=os.path.join(package_share_dir, "Facerec_QT/face_rec_models","inference_model_993.tflite")
        self.fd_model = NetworkExecutor(self.fd_model_dir, preprocess_for_fd)
        self.fe_model = NetworkExecutor(self.fe_model_dir, preprocess_for_fe)
