...
public:
  ...
  PickNPlace() : Node("pick_n_place", rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true)), goal_done_(false)
  {
    ...
    // this->planning_components = std::make_shared<moveit_cpp::PlanningComponent>("arm_robot", this->moveit_cpp_);
    ...
    this->arm_robot_move_server_ = rclcpp_action::create_server<Pnp>(
      this, 
      "pick_n_place_as",
...
private:
  ...
  rclcpp_action::Server<Pnp>::SharedPtr arm_robot_move_server_;
  ...
  void doPoses(std::vector<geometry_msgs::msg::Pose>& poses, moveit_cpp::PlanningComponentPtr planning_components, moveit_cpp::MoveItCppPtr moveit_cpp_ptr){
    ...
    for(unsigned int i = 0; i < poses.size(); i++)
    {
      ...
      // auto planning_components = std::make_shared<moveit_cpp::PlanningComponent>("arm_robot", moveit_cpp_);
...
int main(int argc, char **argv)
{
  ...
  auto arm_robot_action_server = std::make_shared<articubot_core::PickNPlace>();

  rclcpp::spin(arm_robot_action_server);
